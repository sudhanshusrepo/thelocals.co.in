...
import { ProviderProfile, Doc, DocType, DocStatus } from '../../types';
...
const DocUpload: React.FC<{ doc: Doc, onUpdate: (update: Partial<Doc>) => void }> = ({ doc, onUpdate }) => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.show(`Uploaded ${doc.type} successfully!`, { type: 'success' });
        } catch (error) {
            onUpdate({ status: 'empty' });
            toast.show(`Upload failed: ${(error as Error).message}`, { type: 'error' });
        }
...
export const DocumentStep: React.FC<StepProps> = ({ data, updateData, onNext, onBack }) => {
    ...
    const handleDocUpdate = (docType: DocType, updates: Partial<Doc>) => {
        const updatedDocs = {
            ...data.documents,
